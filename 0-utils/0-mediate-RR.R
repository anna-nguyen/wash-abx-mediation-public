##############################################
##############################################
# Documentation: mediate.RR
# Usage: mediate.RR(model.m, model.y, sims = 1000, boot = FALSE,
...
# Description: modifies the mediation::mediate function to compute NIE & NDE RR

# Args/Options:
# same as mediation::mediate

# Returns: ACME & ADE as RR
...
mediate.RR <- function(model.m, model.y, sims = 1000, 
  ...
  if (!isOrdered.y) {
    ...
    if(!boot){
      ...
      for(e in 1:4){
        ...
        #!!!!!!!
        outcome_list[[e]] = c(mean(Pr1), mean(Pr0))
      ...
      tau <- exp((log(zeta.1) + log(delta.0) + log(zeta.0) + log(delta.1))/2)
      nu.0 <- delta.0/tau
      nu.1 <- delta.1/tau
      delta.avg <- exp((log(delta.1) + log(delta.0))/2)
      zeta.avg <- exp((log(zeta.1) + log(zeta.0))/2)
      nu.avg <- (nu.1 + nu.0)/2
